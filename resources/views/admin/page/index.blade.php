...
<div class="flex justify-between items-center mb-4">
  ...
  <div class="space-x-2">
    ...
    <!-- Create Button -->
    <a href="{{ route('page.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700 transition">
      <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  <div class="relative inline-block text-left">
    ...
    <div id="actionDropdown" class="hidden absolute right-0 mt-2 w-40 bg-white border rounded shadow-lg z-50">
      <a href="javascript:void(0);" onclick="return actionSubmit('{{ url('admin/page/update-publish/1') }}');" class="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" role="menuitem">Publish</a>
      <a href="javascript:void(0);" onclick="return actionSubmit('{{ url('admin/page/update-publish/0') }}');" class="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" role="menuitem">Unpublish</a>
      <a href="javascript:void(0);" onclick="return actionConfirm('{{ url('admin/page/bulk_delete') }}','Delete');" class="block px-4 py-2 text-sm text-red-600 hover:bg-red-100" role="menuitem">Delete</a>
    </div>
  </div>
</div>
...
<div class="bg-white shadow rounded-lg p-6">
  <div class="flex justify-between items-center mb-4">
    <h2 class="text-lg font-semibold text-gray-700">Page List</h2>
      ...
      <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
        ...
        <tr>
          ...
          <td class="px-4 py-2 text-right space-x-2">
            <a href="{{ route('page.edit',$m->id) }}" class="px-3 py-1 bg-blue-500 text-white text-xs rounded hover:bg-blue-600">Edit</a>
            {!! $publish[$m->publish] !!}
            <a title="Delete Item" onclick="javascript:return confirm('Are you sure to delete ?')" href="{{ url('admin/page/single-delete', $m->id) }}" class="px-3 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600"><i class="mdi mdi-delete"></i>Delete</a>
          </td>
        </tr>
        @endforeach
