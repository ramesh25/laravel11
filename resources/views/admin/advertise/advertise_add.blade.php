...
<div class="flex justify-between items-center mb-4">
  ...
  <div class="space-x-2">
    <!-- Back Button -->
    <a href="{{ url('admin/advertise') }}" class="inline-flex items-center px-4 py-2 bg-cyan-600 text-white text-sm font-medium rounded hover:bg-cyan-700 transition">
...
<div class="min-h-screen flex items-center justify-center bg-gray-100">
  <form action="{{ route('advertise.store') }}" method="POST" enctype="multipart/form-data" class="w-full max-w-6xl">
    @csrf
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
      ...
      <div class="md:col-span-8">
        <div class="bg-white p-6 rounded shadow">
          <h2 class="text-lg font-semibold text-gray-700 mb-4">Advertise Information</h2>
      ...
      <!-- Link -->
          ...
          <div class="mb-4">
            ...
            <input type="text" name="link" value="{{ old('link') }}" class="w-full border rounded px-4 py-2 focus:ring focus:border-blue-400" >
            @error('link') <p class="text-sm text-red-500 mt-1">{{ $message }}</p> @enderror
          </div>
          <div class="mb-4 md:flex md:items-center">
            <label for="position" class="md:w-1/3 block text-gray-700 text-sm font-medium mb-1 md:mb-0">
              Add Position
            </label>
            <div class="md:w-2/3">
              <select name="position" id="position"
                class="w-full border-gray-300 rounded px-4 py-2 focus:outline-none focus:ring focus:border-blue-400">
                <option value="home_sidebar" {{ old('position', $model->position ?? '') == 'home_sidebar' ? 'selected' : '' }}>Home Sidebar</option>
                <option value="header" {{ old('position', $model->position ?? '') == 'header' ? 'selected' : '' }}>Header</option>
              </select>
              @error('position')
          ...
          <div class="mt-6">
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Save advertise</button>
          </div>
        </div>
      </div>
    </div>
